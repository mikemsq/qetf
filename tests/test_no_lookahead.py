...
def test_data_access_t1_access(tmp_path):
    """Verify DataAccessContext enforces T-1 data access.
    ...
    ohlcv = data_access.prices.read_prices_as_of(as_of=as_of)
    close_prices = ohlcv.xs('Close', level='Price', axis=1)

    # Latest date in data should be 2023-01-09 (day before as_of)
    assert close_prices.index.max() < as_of, "Data includes as_of date (lookahead!)"
    assert close_prices.index.max() == pd.Timestamp('2023-01-09'), "Latest date should be T-1"

    # Latest price should be 108 (day 8), NOT 109 (day 9)
    # This proves we're not seeing data from as_of date
    latest_prices = close_prices.loc[close_prices.index.max()]
    assert all(latest_prices == 108), f"Expected price 108, got {latest_prices.values}"


def test_strict_inequality_t1(tmp_path):
    ...
    assert as_of not in close_prices.index, "as_of date included in data (lookahead!)"

    # Should include day before
    assert pd.Timestamp('2023-01-09') in close_prices.index, "T-1 date should be included"


def test_momentum_alpha_no_lookahead(tmp_path):
...
def test_lookback_window_no_lookahead(tmp_path):
    ...
    ohlcv = data_access.prices.read_prices_as_of(as_of=as_of, lookback_days=10)
    close_prices = ohlcv.xs('Close', level='Price', axis=1)

    # Should have 10 days of data
    assert len(close_prices) == 10, f"Expected 10 days, got {len(close_prices)}"

    # Latest date should be 2023-01-19 (T-1)
    assert close_prices.index.max() == pd.Timestamp('2023-01-19'), \
        f"Latest date should be T-1, got {close_prices.index.max()}"

    # Earliest date should be 2023-01-10 (T-1 minus 9 more days)
    assert close_prices.index.min() == pd.Timestamp('2023-01-10'), \
        f"Earliest date should be 10 days before T, got {close_prices.index.min()}"

    # Should NOT include as_of date
    assert as_of not in close_prices.index, "Lookback window includes as_of date (lookahead!)"


def test_momentum_with_different_lookbacks(tmp_path):
...
def test_edge_case_insufficient_data(tmp_path):
...
def test_no_lookahead_with_multiple_dates(tmp_path):
    ...
    for as_of in test_dates:
        ...
        assert as_of not in close_prices.index, \
        ...
        assert close_prices.index.max() == expected_latest, \
            f"For as_of={as_of}, expected latest={expected_latest}, got {close_prices.index.max()}"

        # Verify price matches expected value
        day_index = (expected_latest - pd.Timestamp('2023-01-01')).days
        expected_price = 100 + day_index
        actual_price = close_prices.loc[expected_latest, 'TICKER_A']
